...
export function startServer({ port = 3001, host = "127.0.0.1", dataDir = process.env.MARGIN_DATA_DIR || resolve(here, "../data"), serveWeb = false } = {}) {
  ...
  if (serveWeb) {
    const webDir = resolve(here, "../dist");
    if (!existsSync(webDir)) throw new Error("Web build not found. Run npm run build first.");
    app.use(express.static(webDir));
    app.use((req,res,next) => req.method === "GET" && req.accepts("html") ? res.sendFile(resolve(webDir,"index.html")) : next());
  }

  const server = app.listen(port,host,()=>console.log(`Margin ready at http://${host}:${port}`));
  const close = () => new Promise(resolveClose => server.close(() => { db.close(); resolveClose(); }));
